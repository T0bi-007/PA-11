...
Replace Oluwatobi Adeyemo with your name.
...
class ImageGraph:
    ...
    def dfs(self, start_index, color):
        ...
        start_vertex = self.vertices[start_index]
        original_color = start_vertex.color
        if original_color == color:  # Avoid redundant coloring
            return
...
def create_graph(data):
    ...
    num_edges = int(lines[2 + num_vert])
    
    # connect vertex A to vertex B and the other way around
    for i in range(3 + num_vert, 3 + num_vert + num_edges):
    ...
    start_index, fill_color = lines[-1].split(", ")
